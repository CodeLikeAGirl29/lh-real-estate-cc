...
export default function BlogPage() {
  return (
    <main className="min-h-screen bg-gray-900 text-white selection:bg-indigo-500/30">
      <Navbar />
      ...
      <div className="relative isolate pt-24 pb-12 px-6 lg:px-8 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-64 opacity-20 pointer-events-none">
          <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[100%] bg-indigo-600 blur-[120px] rounded-full"></div>
        </div>

        <div className="max-w-7xl mx-auto text-center mt-20 mb-16 relative z-10">
          <span className="inline-block px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 text-xs font-bold uppercase tracking-[0.2em] mb-6">
            Insights & Intelligence
          </span>
          <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter">
            The <span className="text-indigo-500 italic">Digital</span> Ledger
          </h1>
        ...
        <div className="max-w-7xl mx-auto grid gap-8 md:grid-cols-2 lg:grid-cols-3 pb-32">
          {posts.map((post) => (
            <article
              key={post.id}
              className="group relative flex flex-col bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden hover:bg-white/[0.04] hover:border-indigo-500/50 transition-all duration-500"
            >
              {/* Image Container */}
              ...
              <div className="p-8 flex flex-col flex-1">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="flex items-center gap-1.5 text-gray-500 text-xs font-mono">
                    <FaCalendarDays className="text-indigo-500" /> {post.date}
                  </span>
                </div>

                <h3 className="text-2xl font-bold mb-4 text-white group-hover:text-indigo-400 transition-colors leading-tight">
                  {post.title}
                </h3>
                ...
                >
                  Read Article <FaArrowRight className="text-indigo-500" />
                </Link>
              </div>
            </article>
          ))}
